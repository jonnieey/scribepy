...
if not BASS_Init(-1, 44100, 0, 0, 0):
    ...
    result_BASS_FXSetParameters = BASS_FXSetParameters(
        DX8_effect_handle, ctypes.pointer(params)
    )
    print("result BASS_FXSetParameters %d" % result_BASS_FXSetParameters)
    result_BASS_FXGetParameters = BASS_FXGetParameters(
        DX8_effect_handle, ctypes.pointer(params)
    )
    ...
    if not BASS_Free():
        print(
            "BASS_Free error %s" % get_error_description(BASS_ErrorGetCode())
        )
