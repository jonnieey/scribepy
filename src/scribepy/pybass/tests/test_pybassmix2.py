...
if __name__ == "__main__":
    if not BASS_Init(1, 44100, 0, 0, 0):
        ...
        print(
            "%s - BASS_Mixer_StreamCreate - %s"
            % (repr(mixer), get_error_description(BASS_ErrorGetCode()))
        )

        BASS_ChannelPlay(mixer, False)
        print(
            "           - BASS_Channel play mixer %s"
            % get_error_description(BASS_ErrorGetCode())
        )
        ...
        stream = BASS_StreamCreateFile(
            False, "test.mp3", 0, 0, BASS_STREAM_PRESCAN | BASS_STREAM_DECODE
        )
        print(
            "%s - BASS_StreamCreateFile %s"
            % (repr(stream), get_error_description(BASS_ErrorGetCode()))
        )
        ...
        print(
            "%s - GET Mixer - %s"
            % (repr(mom), get_error_description(BASS_ErrorGetCode()))
        )

        sync = BASS_ChannelSetSync(
            stream,
            BASS_SYNC_END | BASS_SYNC_ONETIME,
            0,
            SYNCPROC(CloseStream),
            0,
        )
        print(
            "%s - SET Sync close - %s"
            % (repr(sync), get_error_description(BASS_ErrorGetCode()))
        )

        sec10 = BASS_ChannelSeconds2Bytes(stream, 10)
        print(
            "10 seconds = %d bytes, %s"
            % (sec10, get_error_description(BASS_ErrorGetCode()))
        )

        tsync = BASS_ChannelSetSync(
            stream,
        ...
        print(
            "%s - SET Sync 10 sec - %s"
            % (repr(tsync), get_error_description(BASS_ErrorGetCode()))
        )
        ...
        while channel_position < channel_length:
            channel_position = BASS_ChannelGetPosition(stream, BASS_POS_BYTE)
            value = "Play second %s of song" % str(
                int(BASS_ChannelBytes2Seconds(stream, channel_position))
            )
