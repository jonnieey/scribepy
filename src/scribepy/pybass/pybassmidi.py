...
class BASS_MIDI_MARK(ctypes.Structure):
    _fields_ = [
        ("track", ctypes.c_ulong),  # DWORD track; // track containing marker
...
# Additional tag type
BASS_TAG_MIDI_TRACK = (
    0x11000  # + track #, track text : array of null-terminated ANSI strings
)
...
BASS_MIDI_StreamLoadSamples = func_type(ctypes.c_byte, HSTREAM)(
...
BASS_MIDI_FontInit = func_type(HSOUNDFONT, ctypes.c_void_p, ctypes.c_ulong)(
    ("BASS_MIDI_FontInit", bassmidi_module)
)
# BOOL BASSMIDIDEF(BASS_MIDI_FontFree)(HSOUNDFONT handle);
BASS_MIDI_FontFree = func_type(ctypes.c_byte, HSOUNDFONT)(
...
BASS_MIDI_FontCompact = func_type(ctypes.c_byte, HSOUNDFONT)(
...
BASS_MIDI_FontGetVolume = func_type(ctypes.c_float, HSOUNDFONT)(
    ...
    if not pybass.BASS_Init(-1, 44100, 0, 0, 0):
        ...
        if font == 0:
            ...
            if BASS_MIDI_FontGetInfo(font, font_info):
                ...
                print(
                    "name: %s\nloaded: %d / %d"
                    % (font_info.name, font_info.samload, font_info.samsize)
                )
            handle = BASS_MIDI_StreamCreateFile(
                False, b"test.mid", 0, 0, 0, 44100
            )
