...
class slider_ctrl(wx.Slider):
    ...
    def event_timer(self, event):
        if (
            self.player_ctrl.method_get_position()
            < self.player_ctrl.method_get_length() - 1
        ):
...
class player_ctrl(wx.Panel):
    def __init__(self, *args, **kwargs):
        ...
        self.plugins = {}
        self.plugins["aac"] = (
            pybass.BASS_PluginLoad("bass_aac.dll", 0),
            "|AAC|*.aac",
        )
        self.plugins["ac3"] = (
            pybass.BASS_PluginLoad("bass_ac3.dll", 0),
            "|AC3|*.ac3",
        )
        self.plugins["aix"] = (
            pybass.BASS_PluginLoad("bass_aix.dll", 0),
            "|AIX|*.aix",
        )
        self.plugins["ape"] = (
            pybass.BASS_PluginLoad("bass_ape.dll", 0),
            "|APE|*.ape",
        )
        self.plugins["mpc"] = (
            pybass.BASS_PluginLoad("bass_mpc.dll", 0),
            "|MPC|*.mpc",
        )
        self.plugins["ofr"] = (
            pybass.BASS_PluginLoad("bass_ofr.dll", 0),
            "|OFR|*.ofr",
        )
        self.plugins["spx"] = (
            pybass.BASS_PluginLoad("bass_spx.dll", 0),
            "|SPX|*.spx",
        )
        self.plugins["tta"] = (
            pybass.BASS_PluginLoad("bass_tta.dll", 0),
            "|TTA|*.tta",
        )
        self.plugins["cda"] = (
            pybass.BASS_PluginLoad("basscd.dll", 0),
            "|CDA|*.cda",
        )
        self.plugins["flac"] = (
            pybass.BASS_PluginLoad("bassflac.dll", 0),
            "|FLAC|*.flac",
        )
        self.plugins["wma"] = (
            pybass.BASS_PluginLoad("basswma.dll", 0),
            "|WMA, WMV|*.wma;*.wmv",
        )
        if pybassmidi:
            ...
            if self.sound_font == 0:
                print(
                    ...
                    % (
                        ...
                        sound_font_file_name,
                    )
                )
            else:
                self.plugins["midi"] = (
                    pybass.BASS_PluginLoad("bassmidi.dll", 0),
                    "|MID|*.mid",
                )
        else:
        ...
        self.btn_play = wx.Button(
            self, wx.ID_ANY, _("Play"), style=wx.NO_BORDER
        )
        ...
        self.volume_slider = wx.Slider(
            self, wx.ID_ANY, pybass.BASS_GetVolume() * 100, 0, 100
        )
    ...
    def method_load_data(self, stream, name_stream="memory_stream"):
        if stream is not None:
            ...
            if isinstance(self.stream, memory_stream):
                ...
                if new_bass_handle == 0:
                    print(
                        "BASS_StreamCreateFileUser error %s"
                        % pybass.get_error_description(
                            pybass.BASS_ErrorGetCode()
                        )
                    )
    ...
    def method_get_length(self):
        result = pybass.BASS_ChannelGetLength(
            self.bass_handle, pybass.BASS_POS_BYTE
        )
    ...
    def method_get_position(self):
        return pybass.BASS_ChannelGetPosition(
            self.bass_handle, pybass.BASS_POS_BYTE
        )

    def method_set_position(self, value):
        if not pybass.BASS_ChannelSetPosition(
            self.bass_handle, value, pybass.BASS_POS_BYTE
        ):
            print(
                "BASS_ChannelSetPosition error %s"
                % pybass.get_error_description(pybass.BASS_ErrorGetCode())
            )
        ...
        if self.bass_handle:
            ...
            if hasattr(self.stream, "name"):
                text = (
                    self.stream.name
                    + " ("
                    + pybass.seconds_to_string(self.stream.seconds)
                    + ")"
                )
            else:
                # ~ channel_info = self.method_get_channel_info()
                ...
                text = (
                    self.name_stream
                    + " ("
                    + pybass.stream_length_as_hms(self.bass_handle)
                    + ")"
                )
    ...
    def method_is_end(self):
        return (
            self.method_get_state() == pybass.BASS_ACTIVE_STOPPED
            and self.method_get_position() == 0
        )

    def method_play(self):
        if self.bass_handle:
            if self.method_get_state() in (
    ...
    class main_frame(wx.Frame):
        def __init__(self, *args, **kwargs):
            ...
            self.log_ctrl = log_ctrl(
                self, style=wx.TE_MULTILINE, add_to_file=True
            )
            ...
            self.aui_manager.AddPane(
                self.player, AuiPaneInfo().Name("player").CenterPane()
            )
            self.aui_manager.AddPane(
                self.log_ctrl,
                AuiPaneInfo()
        ...
        def start(self):
            ...
            if os.path.exists("lang/%s" % lang_catalog):
                for dir_name in os.listdir("lang"):
                    if os.path.exists(
                        "lang/%s/%s.mo" % (dir_name, self.app_name)
                    ):
                        if dir_name == lang_catalog:
                            current_trans = i
                            self.help_file = (
                                "lang/" + dir_name + "/" + self.help_file
                            )
                        list_trans.append(
            ...
            self.email = (
                "mailto:pyirrlicht@gmail.com",
                _("email for feedback"),
            )
            self.license = _("BSD license")
            self.about_description = _("wxPython bass music player.")
            # CREATE MAIN FRAME
            self.main_frame = main_frame(
                None, wx.ID_ANY, self.app_name, app=self
            )
