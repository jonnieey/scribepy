...
error_descriptions[
    BASS_ERROR_INIT
] = "BASS_ASIO_Init has not been successfully called"
BASS_ERROR_START = 9
...
BASS_ASIO_GetVersion = func_type(ctypes.c_ulong)(
    ("BASS_ASIO_GetVersion", bassasio_module)
)
# DWORD BASSASIODEF(BASS_ASIO_ErrorGetCode)();
BASS_ASIO_ErrorGetCode = func_type(ctypes.c_ulong)(
...
BASS_ASIO_SetDevice = func_type(ctypes.c_byte, ctypes.c_ulong)(
    ("BASS_ASIO_SetDevice", bassasio_module)
)
# DWORD BASSASIODEF(BASS_ASIO_GetDevice)();
BASS_ASIO_GetDevice = func_type(ctypes.c_ulong)(
    ("BASS_ASIO_GetDevice", bassasio_module)
)
# BOOL BASSASIODEF(BASS_ASIO_Init)(DWORD device);
BASS_ASIO_Init = func_type(ctypes.c_byte, ctypes.c_ulong)(
...
if __name__ == "__main__":
    ...
    if not BASS_ASIO_Init(0):
        print(
            "BASS_ASIO_Init error %s"
            % get_error_description(BASS_ASIO_ErrorGetCode())
        )
    else:
        if not BASS_ASIO_Free():
            print(
                "BASS_ASIO_Free error %s"
                % get_error_description(BASS_ASIO_ErrorGetCode())
            )
